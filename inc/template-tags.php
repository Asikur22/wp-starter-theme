...
function wp_bootstrap_starter_posted_on() {
    ...
    if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
        ...
        comments_popup_link( sprintf( wp_kses( __( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', AA_THEME_SLUG ), array( 'span' => array( 'class' => array() ) ) ), get_the_title() ) );
        echo '</span>';
    }
...
function wp_bootstrap_starter_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		$categories_list = get_the_category_list( esc_html__( ', ', AA_THEME_SLUG ) );
		if ( $categories_list && wp_bootstrap_starter_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', AA_THEME_SLUG ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', AA_THEME_SLUG ) );
		if ( $tags_list ) {
			printf( ' | <span class="tags-links">' . esc_html__( 'Tagged %1$s', AA_THEME_SLUG ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
	...
	edit_post_link(
		sprintf(
			...
			esc_html__( 'Edit %s', AA_THEME_SLUG ),
       ...
       // $GLOBALS['comment'] = $comment;
        ...
        if ( 'pingback' == $comment->comment_type || 'trackback' == $comment->comment_type ) : ?>
            ...
            <div class="comment-body">
                <?php _e( 'Pingback:', AA_THEME_SLUG ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( 'Edit', AA_THEME_SLUG ), '<span class="edit-link">', '</span>' ); ?>
            </div>
                ...
                <div class="media-body">
                    <div class="media-body-wrap card">
                        ...
                        <div class="card-header">
                            <h5 class="mt-0"><?php printf( __( '%s <span class="says">says:</span>', AA_THEME_SLUG ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?></h5>
                            <div class="comment-meta">
                                ...
                                <?php edit_comment_link( __( '<span style="margin-left: 5px;" class="glyphicon glyphicon-edit"></span> Edit', AA_THEME_SLUG ), '<span class="edit-link">', '</span>' ); ?>
                            </div>
                        </div>
